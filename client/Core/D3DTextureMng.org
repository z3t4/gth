...
bool CTextureResource::UploadTexture()
{
	...
	if( imageinfo.ImageFileFormat == D3DXIFF_DDS )
	{
		...
		hr = D3DXCreateTextureFromFileInMemoryEx(
			...
			D3DX_FILTER_NONE ,//D3DX_FILTER_BOX ,//| D3DX_FILTER_DITHER,
			0 ,
			&info ,
			NULL ,
			&texture->m_d3dTex );
...
CD3DTextureMng::CD3DTextureMng()
{
	...
	// 2004-07-09 16:20, Added by yonghak
...
// after all this called, Reset must called
...
// Restore is Release and LoadAll
#include <assert.h>
void CD3DTextureMng::Restore()
{
	CTextureResource	*texture = NULL;
	char		texName[256];
	char		fName[256];
	char		*ext;
	...
	{
		// 2004-11-11 16:46, elfshadow begin
		assert( i < MAX_SIZE_TEXTURE );
		// 2004-11-11 16:46, elfshadow end		
...
/*		if( m_texQuality == GTH_TEXTURE_QUALITY_LOW && 
			( texture->m_owner != OWNER_TYPE_INTERFACE && texture->m_owner != OWNER_TYPE_BSP_LIGHTMAP ) )
		{
			sprintf( texName , "low_" );
			strcat( texName , texture->m_name );
		}
		else
		{
		...
		// 2004-11-11 16:33, elfshadow begin
		if ( texture == NULL)
			continue;
		// 2004-11-11 16:33, elfshadow end
		...
		if( !g_fileMng.RegisterFileFromPack( texture->m_name, (RHANDLE)texture, texture, RES_INIT_NOTCREATE ) )
		{
			...
			if( !g_fileMng.RegisterFileFromPack( texture->m_name, (RHANDLE)texture, texture, RES_INIT_NOTCREATE ) )
			{
				...
				if( !g_fileMng.RegisterFileFromPack( texture->m_name, (RHANDLE)texture, texture, RES_INIT_NOTCREATE ) ) 
				{
//					ShowLogInfo( "TEXTURE(%s : register error!)", texture->m_name );	
					continue;
				}
			}
		}
...
LPDIRECT3DTEXTURE8 CD3DTextureMng::GetD3DTexture( int texRef )
{
	...
	{
//		ShowLogInfo( "TEXTURE(%s : recreate error!)", texture->m_name );
		return NULL;	
	}

	if( bIsDisposed )
	{
//		ShowLogInfo( "TEXTURE(%s : recrete!)", texture->m_name );
	}

...
void CChatManager::SetCombi( HWND hwnd, HIMC hIMC )
{
...
int CChatManager::GetString( HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_CHAR :
		...
		if( wParam == VK_TAB ) 
		{
			return( CHAT_INPUT_RETURN_TAB );
		}
		if( wParam == VK_ESCAPE ) 
		{
			return( CHAT_INPUT_RETURN_EXIT );
		}
		if( wParam == VK_BACK )			
		{
			BackSpace();
		...
		else
		{
			...
			if( !(GetKeyState( VK_CONTROL ) < 0 ) && wParam != 13 )
			{
				AddChar( (BYTE)wParam );
			}
		}
